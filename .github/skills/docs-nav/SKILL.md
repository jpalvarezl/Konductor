...
1. For implementation work, open the referenced GitHub issue and its linked
   **`docs/iterations/I###-*.md`** packet directly when one exists. Do not read `docs/iterations/index.md` first when
   the packet is already known.
   - The issue owns coordination, assignment, dependencies, blockers, status, and closure.
   - A packet is a small implementation brief/context pack for accepted non-trivial work. It owns scope, non-goals,
     acceptance, exact context, constraints, and validation—not current status.
   - Follow only its linked spec headings, source symbols, tests, and targeted searches.
   - Design/triage issues and tiny obvious fixes may intentionally be packet-free; use their exact references and the
     owning spec rather than inventing a packet.
   - If GitHub is unavailable, continue from a linked self-contained packet.
   - Do not scan all of `docs/` or `src/` unless the targeted context is incomplete or contradicted by source.
...
4. For unscheduled direction, use **`docs/future.md`**. Create a focused issue when an idea needs discussion or
   prioritization; create a delivery packet only after non-trivial work is accepted, scoped, and ready for an offline
