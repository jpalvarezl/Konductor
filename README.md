...
- Press `/` in an empty composer or `Ctrl+K` while idle to open the command palette. Type to fuzzy-filter,
  use `Up` / `Down` to select, `Enter` to stage a command for confirmation, and `Esc` to close it. If `/` opened the
  palette, `Esc` restores the full slash-prefixed draft (for example `/etc/hosts`) instead of discarding it.
