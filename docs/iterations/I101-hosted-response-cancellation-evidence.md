...
stream close still lose cancellation at the Azure `HttpClient` subscription seam, but Kotlin
`runInterruptible(Dispatchers.IO)` can interrupt the thread blocked in Azure's synchronous `HttpPipeline.sendSync`.
Reactor then cancels the active Azure `HttpClient` publisher subscription. `AzureHostedAgentClient.invoke` uses that
path and restores caller `CancellationException` semantics when Azure/Reactor wraps the carrier-thread
...
- **Async unary:** `buildAgentScopedOpenAIAsyncClient(String)` exposes
  ...
  subscription, but that inner future is not returned. Cancelling the public dependent future does not cancel its
  parent.
- **Async streaming before headers:** `createStreaming(...)` uses the same dependent transport chain and wraps it
  ...
  `whenComplete { streamResponse?.close() }`; before headers it neither cancels that future nor reaches the injected
  Azure `HttpClient` subscription within the observation window.
...
| Production sync unary through `HostedProvider` | cancel the collecting caller and boundedly join it | caller receives `CancellationException`; Azure `HttpClient` `onCancel` arrives within 2 seconds |
...
`/agents/{agentName}/endpoint/protocols/openai/responses?api-version=v1` and the exact Hosted session property. With
`AgentsClientBuilder`'s default retry pipeline, the delayed first call produces one `HttpClient.send` invocation and
one publisher subscription, and no additional request is observed during the two-second post-cancellation bound. The
fixture emits neither a response nor an error before cancellation, so it does not trigger a retry condition; this is
not a no-retry configuration or a claim that every failure mode is single-send.
