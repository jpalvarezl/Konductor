...
`IterableStream`. The Hosted sample and current live-verified path still use an agent-scoped OpenAI client and an
untyped session property.
...
The embedded openai-java `ResponseService` also exposes a separate `cancel(responseId)` request, and the sync wrapper
makes it reachable through `getResponseService()`. It requires a response ID and is distinct from closing or cancelling
the local in-flight call. Konductor does not invoke it, and its service applicability to Konductor's current foreground
Foundry requests is **unknown**.
...
### Azure-specific options, structured inputs, and agent references
...
streaming with both fields, and the structured-input sample uses `BinaryData.fromObject(...)`. These are structured
**prompt inputs**, not openai-java's typed structured-output response facility.

Konductor has no structured-input domain field today. If a product scenario adds one, scope it as a separate adapter
change, preserve arbitrary JSON values, and construct each value with `BinaryData.fromObject` rather than assuming
that JSON text supplied through `fromString` will be embedded as raw JSON.
...
azure-ai-agents 2.2.0 by deterministic root-client and stream ownership, and remains required by the pinned Hosted
sample shape.
...
- Underlying 4.14.0 lifecycle: openai-java [`OpenAIClient`][openai-client],
  [`ClientOptions`][openai-options], and [`AsyncStreamResponse`][openai-async-stream]
